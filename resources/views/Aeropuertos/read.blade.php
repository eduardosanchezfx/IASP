...
<div class="content-wrapper">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12 text-center">
            <h1 class="m-0 text-dark"><i class="fas fa-plane-departure"></i> Administracion de Aeropuertos</h1>
          </div><!-- /.col -->
    ...
    <div class="card">
      <div class="card-header">
        <h3 class="card-title"><i class="fas fa-list-ul"></i> Lista de Aeropuertos</h3>
        <div class="card-tools">
            @if(auth()->user()->level=='S')
            <a href="/Crear_Aeropuerto" class="btn btn-tool btn-sm" data-toggle="tooltip" data-placement="top" title="Crear Aeropuerto" >
          ...
          <tr class="text-center">
            ...
            <td>
              <div class="row text-center justify-content-center"> 
                  ...
                  @if(($almacen->deleted_at)!=null)
                  <form method="post" action="{{url('Aeropuertos/'.$almacen->id)}}">
                @csrf
                 {{method_field('DELETE')}}
              <button type="submit" onclick="return confirm('¿Deseas Eliminar?, Esta accion es permanente')" class="btn  btn-outline-danger" data-toggle="tooltip" data-placement="top" title="Eliminar Permanentemente {{$almacen->nombre}}"><i class="fas fa-trash"></i></button>
                ...
                <a href="{{ route('Aeropuertos.edit', $almacen->id) }}" class="btn btn-outline-success" ><i class="fas fa-trash-restore" data-toggle="tooltip" data-placement="top" title="Restaurar {{$almacen->nombre}}"></i></a>
                @endif
            @if($almacen->deleted_at==null)
            <form method="post" action="{{url('Aeropuertos/'.$almacen->id)}}">
                @csrf
            ...
            <a href="{{ route('Aeropuertos.edit',$almacen->id)}}" class=" btn btn-outline-warning" data-toggle="tooltip" data-placement="top" title="Editar a {{$almacen->nombre}}"><i class="far fa-edit"></i></a>
